...
namespace DK.Code
{
    public struct CodeSpan
    {
        ...
        public CodeSpan First(int count)
        {
            if (IsEmpty) return CodeSpan.Empty;
            if (_start + count >= _end) return this;
            return new CodeSpan(_start, _start + count);
        }

        public CodeSpan Last(int count)
        {
            if (IsEmpty) return CodeSpan.Empty;
            if (_end - count <= _start) return this;
            return new CodeSpan(_end - count, _end);
        }
    }
}
