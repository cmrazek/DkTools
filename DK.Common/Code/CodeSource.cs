...
namespace DK.Code
{
	public class CodeSource : IPreprocessorWriter
	{
		...
		public CodeSpan GetFileSpan(CodeSpan sourceSpan, out string fileNameOut, out bool isInPrimaryFile)
		{
			...
			if (seg.fileStartPos + seg.length >= sourceSpan.End)
			{
				...
				if (seg.actualContent) return new CodeSpan(startPos, startPos + sourceSpan.Length);
				else return new CodeSpan(seg.startPos, seg.startPos);
			}
			...
			for (int s = startSegIndex + 1; s <= endSegIndex; s++)
			{
				...
				if (seg.fileStartPos > sourceSpan.End) return new CodeSpan(startPos, endPos);
				if (seg.fileStartPos + seg.length >= sourceSpan.End)
				{
					return new CodeSpan(startPos, seg.actualContent ? seg.startPos + (sourceSpan.End - seg.fileStartPos) : seg.startPos);
				}

				endPos = seg.actualContent ? seg.startPos + seg.length : seg.startPos;
			}

			return new CodeSpan(startPos, endPos);
		}
		...
		public void GetFileSpan(CodeSpan sourceSpan, out string foundFileName, out CodeSpan foundSpan, out bool foundPrimaryFile)
		{
			...
			if (startLocalPos.FileName.Equals(endLocalPos.FileName, StringComparison.OrdinalIgnoreCase) && endLocalPos.Position >= startLocalPos.Position)
			{
				...
				foundSpan = new CodeSpan(startLocalPos.Position, endLocalPos.Position);
				foundPrimaryFile = startLocalPos.PrimaryFile;
			}
			else
			{
				foundFileName = startLocalPos.FileName;
				foundSpan = new CodeSpan(startLocalPos.Position, startLocalPos.Position);
				foundPrimaryFile = startLocalPos.PrimaryFile;
			}
		}
		...
		public IEnumerable<CodeSpan> GenerateDisabledSections()
		{
			var disabled = false;
			var disableStart = 0;
			...
			foreach (var seg in _segments)
			{
				...
				{
					yield return new CodeSpan(disableStart, seg.startPos);
					disabled = false;
				}
			}
			...
			if (disabled)
			{
				if (_segments.Count > 0) yield return new CodeSpan(disableStart, _segments[_segments.Count - 1].endPos);
			}
		}
