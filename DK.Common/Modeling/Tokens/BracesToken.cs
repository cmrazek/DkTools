...
{
	public class BracesToken : GroupToken
	{
		...
		internal BracesToken(Scope scope)
			: base(scope)
		{
		}

		internal BracesToken(Scope scope, CodeSpan openBraceSpan)
			: base(scope)
		{
		...
		internal static BracesToken Parse(Scope scope, Definition ownerDef, int funcOutliningStart = -1)
		{
			if (ownerDef != null)
			{
			...
			get
			{
				if (_openToken != null && _closeToken != null)
				{
					...
					if (_closeToken.Span.Start > startLineEnd)
					{
						...
						{
							span = new CodeSpan(_funcOutliningStart, Span.End);
							collapse = true;
						}
