...
{
	public class IncludeToken : GroupToken
	{
		...
		private IncludeToken(Scope scope, CodeSpan span, PreprocessorToken prepToken, string fileName, bool searchFileDir)
			: base(scope)
		{
		...
		internal static IncludeToken Parse(Scope scope, PreprocessorToken prepToken)
		{
			var code = scope.Code;
			code.SkipWhiteSpace();
			...
			CodeSpan includeSpan = prepToken.Span;
			...
			if (code.ReadIncludeStringLiteral())
			{
				...
				includeSpan = new CodeSpan(prepToken.Span.Start, fileNameToken.Span.End);
