...
{
    public class ExpressionToken : GroupToken
    {
        ...
        public delegate Token WordTokenCallback(string word, CodeSpan wordSpan);

        internal static ExpressionToken TryParse(Scope scope, IEnumerable<string> endTokens,
        ...
        private static Token ProcessWord(ExpressionToken exp, Scope scope, string word, CodeSpan wordSpan)
        {
            ...
            else if (code.PeekExact("$$"))
            {
                ...
                if (!string.IsNullOrEmpty(word2))
                {
                    ...
                    foreach (var def in scope.DefinitionProvider.GetAny(wordSpan.Start, word)
                        .Where(x => x.AllowsDoubleDollarChild))
                    {
                        var childDef = def.GetDoubleDollarChildDefinitions(word2, scope.AppSettings).FirstOrDefault();
                        if (childDef == null) continue;

                        var word1Token = new IdentifierToken(scope, wordSpan, word, def);
                        var dollarToken = new DoubleDollarToken(scope, dollarSpan);
                        var word2Token = new IdentifierToken(scope, word2Span, word2, childDef);
                        var compToken = new CompositeToken(scope, childDef.DataType);
                        compToken.AddToken(word1Token);
