...
namespace DK.Modeling
{
    public sealed class FileStore
    {
            ...
            IEnumerable<IncludeDependency> includeDependencies = null)
        {
            ...
            var prepModel = new PreprocessorModel(
                appSettings: appSettings,
                source: prepSource,
                definitionProvider: defProvider,
                fileName: fileName,
                visible: visible,
                includeDependencies: prep.IncludeDependencies,
                scanMode: scanMode,
                cancel: cancel)
            {
                Preprocessor = prep,
        ...
        private static void CreateStdLibModel(DkAppContext app, CodeScanMode scanMode)
        {
            ...
            if (includeFile != null)
            {
                _stdLibModel = tempStore.CreatePreprocessedModel(
                    appSettings: app.Settings,
                    source: includeFile.GetSource(app.Settings),
                    fileName: includeFile.FullPathName,
                    visible: false,
                    reason: "stdlib.i model",
                    scanMode: scanMode,
        ...
        public static CodeModel GetStdLibModel(DkAppContext app, CodeScanMode scanMode)
        {
            if (_stdLibModel == null) CreateStdLibModel(app, scanMode);
            return _stdLibModel;
        }
