	...
	internal class ProbeSmartIndent : ISmartIndent
	{
		...
		int? ISmartIndent.GetDesiredIndentation(ITextSnapshotLine line)
		{
			...
			else if (_rxCaseLine.IsMatch(lineTextTrim))
			{
				...
				var model = CodeModel.FileStore.GetOrCreateForTextBuffer(_view.TextBuffer).GetModelForSnapshotOrNewer(_view.TextBuffer.CurrentSnapshot, "Smart indenting - case inside switch");
